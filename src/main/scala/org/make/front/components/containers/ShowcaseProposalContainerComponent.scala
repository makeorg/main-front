...
import org.make.front.components.presentationals.ShowcaseProposalComponent
import org.make.front.models._

object ShowcaseProposalContainerComponent {

  final case class ShowcaseProposalContainerProps(translationKey: String)
  ...
  lazy val reactClass: ReactClass = ReactRedux.connectAdvanced(selectorFactory)(ShowcaseProposalComponent.reactClass)

  def selectorFactory
    : (Dispatch) => (AppState, Props[ShowcaseProposalContainerProps]) => ShowcaseProposalComponent.ShowcaseProps =
    (_: Dispatch) => { (appState: AppState, ownProps: Props[ShowcaseProposalContainerProps]) =>
      def searchThemeByThemeId(themeId: ThemeId): Option[Theme] = {
        appState.themes.find(_.id == themeId)
      }

      ShowcaseProposalComponent.ShowcaseProps(
        proposals = proposals,

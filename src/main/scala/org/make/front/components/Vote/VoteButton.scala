...
import org.make.front.components.Vote.VoteComponent.{Button, VoteProps, VoteState}
import org.make.front.components.Vote._
...
object VoteButtonComponent {
  ...
  def displayQualification(self: Self[VoteButtonProps, Unit]): ReactElement = {
    <.QualificateVoteComponent(
      ^.wrapped := QualificateVoteComponent
        .QualificateVoteProps(voteType = self.props.wrapped.voteType, qualifStats = self.props.wrapped.vote)
    )()
  }

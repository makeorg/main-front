package org.make.front.components.connectUser
...
import org.make.front.components.connectUser.ConnectUser.{ConnectUserProps, ConnectUserState}
...
object ConnectUserContainer extends RouterProps with UserServiceComponent {
  ...
  lazy val reactClass: ReactClass = ReactRedux.connectAdvanced(selectorFactory)(ConnectUser.reactClass)

  def selectorFactory: (Dispatch) => (AppState, Props[Unit]) => ConnectUser.ConnectUserProps =
    (dispatch: Dispatch) =>
      (appState: AppState, _: Props[Unit]) => {
        ...
        def handleFutureApiSignInResponse(futureUser: Future[UserModel],
        ...
        def handleFutureApiRegisterResponse(futureUser: Future[UserModel],
                                            child: Self[ConnectUserProps, ConnectUserState]): Unit = {
          futureUser.onComplete {
            case Success(user) =>
              dispatch(LoggedInAction(user))

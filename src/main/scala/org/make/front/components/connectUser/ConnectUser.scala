...
object ConnectUser {
  ...
  def signInElement(self: Self[ConnectUserProps, ConnectUserState]): ReactElement = {
    ...
    if (self.props.wrapped.isProposalFlow) {
      submitButtonContainer = Seq(ConnectUserStyles.SubmitProposalFormButtonContainer)
      submitButton =
        Seq(MakeStyles.Button.default, ConnectUserStyles.submitButton, ConnectUserStyles.SubmitProposalFormButton)
...
object ConnectUserStyles extends StyleSheet.Inline {
  ...
  val SubmitProposalFormButtonContainer: StyleA = style(display.flex, flexDirection.column)
  val SubmitProposalFormButton: StyleA = style(width(15.4F.rem), margin(0.rem, auto, 1.9F.rem, auto))

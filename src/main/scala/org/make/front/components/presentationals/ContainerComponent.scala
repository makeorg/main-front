...
import org.make.front.components.containers.{ActivateAccountContainerComponent, PasswordResetContainerComponent, ThemeContainerComponent}
    ...
    render = (_) =>
      <.div(^.className := AppComponentStyles.wrapper)(
        <.Switch()(
          ...
          <.Route(^.exact := true, ^.path := "/account-activation/:userId/:verificationToken", ^.component := ActivateAccountContainerComponent.reactClass)(),

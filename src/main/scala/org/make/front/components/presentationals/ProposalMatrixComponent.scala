...
object ProposalMatrixComponent {
    ...
    for (i <- 1 to 20)
      yield
        Proposal(
          ...
          tags = Seq(
            Tag(TagId("tag-un"), label = "un"),
            Tag(TagId("tag-deux"), label = "deux"),
            Tag(TagId("tag-trois"), label = "trois")
          )
        )
  }
  ...
  lazy val reactClass: ReactClass = React.createClass[Unit, Unit](
    render = (self) =>
      <.div()(
        <.h2(^.className := AppComponentStyles.title2)(<.Translate(^.value := "content.theme.matrix.title")()),
        <.TagFilterComponent()(),
        <.div(^.className := ProposalMatrixStyles.proposalList)(
          <.div(^.className := Seq(BulmaStyles.Grid.Columns.columns, BulmaStyles.Grid.Columns.columnsMultiLines))(
            proposals.map(
              proposal =>
                <.ProposalTileComponent(^.wrapped := ProposalTileComponent.ProposalTileProps(proposal = proposal))()
            )
          )
        ),

...
object ConsultationHeader {
  ...
  case class ConsultationHeaderProps(operation: OperationModel, language: String)

  lazy val reactClass: ReactClass =
    React
      .createClass[ConsultationHeaderProps, ConsultationHeaderState](
        displayName = "ConsultationHeader",
        getInitialState = {self => ConsultationHeaderState("consultation")},
        render = { self =>
          ...
          object DynamicConsultationHeaderStyles extends StyleSheet.Inline {
            ...
            val titleWidth: StyleA =
              style(
                maxWidth(consultation.logoWidth.pxToEm())
              )
          ...
          <.div(^.className :=  DynamicConsultationHeaderStyles.gradient)(
            ...
            <.div(^.className := js.Array(LayoutRulesStyles.centeredRow, ConsultationHeaderStyles.tabWrapper))(
              <.button(^.className := js.Array(ConsultationHeaderStyles.tab, ConsultationHeaderStyles.tabSelection(self.state.activeTab == "consultation")), ^.onClick := changeTab("consultation"))(
                ...
                <.span(^.className := ConsultationHeaderStyles.dateLink)(
                  ...
                  " " +
                  unescape(I18n.l(consultation.startDate.getOrElse(new js.Date()), DateLocalizeOptions("common.date.long"))) +
                  " " +
                  unescape(I18n.t("operation.intro.to")) +
                  " " +
                  unescape(I18n.l(consultation.endDate.getOrElse(new js.Date()), DateLocalizeOptions("common.date.long")))
                )
              ),
              <.button(^.className := js.Array(
...
object ConsultationHeaderStyles extends StyleSheet.Inline {
  ...
  val tab: StyleA =
    style(
      ...
      ThemeStyles.MediaQueries.beyondLarge(
        unsafeChild("span")(
          display.inlineBlock,
          paddingLeft(5.pxToEm()),
          paddingRight(5.pxToEm())
        )
      ),
      &.before(
        content := "''",
  ...
  val titleLink: StyleA =
    style(
      TextStyles.title,
      fontSize(16.pxToEm()),
      ThemeStyles.MediaQueries.beyondMedium(fontSize(18.pxToEm())),
    )

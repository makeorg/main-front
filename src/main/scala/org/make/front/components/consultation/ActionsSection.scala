...
import org.make.front.facades.{I18n, actions, actions2x, actions3x, clapping, logoMake}
...
object ActionsSection {
  ...
  case class ActionsSectionProps(operation: OperationExpanded,
                                 language: String,
                                 isConnected: Boolean)
  case class ActionsSectionState(isAuthenticateModalOpened: Boolean,
                                 loginOrRegisterView: String = "register")

  lazy val reactClass: ReactClass =
    React
      .createClass[ActionsSectionProps, ActionsSectionState](displayName = "ActionsSection", getInitialState = {
        _ =>
          ...
          <.article(^.className := ActionsSectionStyles.main)(
            <.div(^.className := js.Array(
              ...
              <.h3(^.className := js.Array(
                ...
                ActionsSectionStyles.title)
              )(
                <.img(
                  ^.className := ActionsSectionStyles.logo,
              ...
              if (!self.props.wrapped.isConnected) {
                <.p(^.className := js.Array(
                  TextStyles.smallerText,
                  ActionsSectionStyles.presentationBait)
                )(
            ...
            <.div(^.className := js.Array(
              ActionsSectionStyles.wrapperAlt,
              LayoutRulesStyles.centeredRow)
            )(
              <.h3(^.className := js.Array(
                TextStyles.smallerTitle,
                ...
                <.img(
                  ^.className := ActionsSectionStyles.icon,
                  ^.src := clapping.toString,
                  ^.alt := "Make.org"
                )(),
            ...
            <.div(^.className := js.Array(
              ...
              <.p(^.className := js.Array(
                ...
                <.ul(^.className := ActionsSectionStyles.presentationText)(
                  <.li(^.className := ActionsSectionStyles.presentationListItem)(unescape(I18n.t("operation.actions.actions-list-item1"))),
                  <.li(^.className := ActionsSectionStyles.presentationListItem)(unescape(I18n.t("operation.actions.actions-list-item2"))),
                  <.li(^.className := ActionsSectionStyles.presentationListItem)(unescape(I18n.t("operation.actions.actions-list-item3")))
                )
              )
            ),
            <.div(^.className := js.Array(

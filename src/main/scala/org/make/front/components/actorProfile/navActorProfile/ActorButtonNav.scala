...
object ActorButtonNav {

  final case class ActorButtonNavProps(onClickMethod: () => Unit, icon: StyleA, wording: String)

  val reactClass: ReactClass =
    React
      .createClass[ActorButtonNavProps, Unit](
        displayName = "ActorButtonNav",
        render = self => {

          <("ActorButtonNav")()(
            <.button(
              ^.className := js
                .Array(CTAStyles.basic, CTAStyles.basicOnButton, ActorButtonNavStyles.slidingPannelGreyButton),
              ^.onClick := self.props.wrapped.onClickMethod
            )(
              <.i(
                ^.className := js
                  .Array(self.props.wrapped.icon, ActorButtonNavStyles.slidingPannelButtonIcon)
              )(),
              <.span()(self.props.wrapped.wording)
            ),
            <.style()(ActorButtonNavStyles.render[String])
          )
        }
      )
}

object ActorButtonNavStyles extends StyleSheet.Inline {
  import dsl._

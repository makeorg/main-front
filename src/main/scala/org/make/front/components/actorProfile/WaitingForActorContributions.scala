...
object WaitingForActorContributions {

  lazy val reactClass: ReactClass =
    React
      .createClass[Unit, Unit](
        displayName = "WaitingForActorContributions",
        render = self => {

          <.div(
            ^.className := js.Array(WaitingForActorContributionsStyles.wrapper, TableLayoutStyles.fullHeightWrapper)
          )(
            <.div(^.className := TableLayoutStyles.row)(
              <.div(
                ^.className := js.Array(TableLayoutStyles.cell, WaitingForActorContributionsStyles.mainHeaderWrapper)
              )(
                <.div(^.className := RWDHideRulesStyles.invisible)(<.CookieAlertContainerComponent.empty),
                <.div(^.className := WaitingForActorContributionsStyles.fixedMainHeaderWrapper)(
            ...
            <.div(^.className := TableLayoutStyles.row)(
              <.div(
                ^.className := js
                  .Array(WaitingForActorContributionsStyles.content, TableLayoutStyles.cellVerticalAlignMiddle)
              )(<.SpinnerComponent.empty)
            ),
            <.style()(WaitingForActorContributionsStyles.render[String])
          )
        }
      )
}

object WaitingForActorContributionsStyles extends StyleSheet.Inline {

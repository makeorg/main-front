...
object WaitingForUserProfileProposals {

  lazy val reactClass: ReactClass =
    React
      .createClass[Unit, Unit](
        displayName = "WaitingForUserProfileProposals",
        render = _ => {

          <.div(^.className := TableLayoutStyles.fullHeightWrapper)(
            <.div(
              ^.className := js
                .Array(WaitingForUserProfileProposalsStyles.content, TableLayoutStyles.cellVerticalAlignMiddle)
            )(<.SpinnerComponent.empty),
            <.style()(WaitingForUserProfileProposalsStyles.render[String])
          )
        }
      )
}

object WaitingForUserProfileProposalsStyles extends StyleSheet.Inline {
  import dsl._

...
object WaitingForUserLikeItProposals {

  lazy val reactClass: ReactClass =
    React
      .createClass[Unit, Unit](
        displayName = "WaitingForUserLikeItProposals",
        render = _ => {

          <.div(^.className := TableLayoutStyles.fullHeightWrapper)(
            <.div(
              ^.className := js
                .Array(WaitingForUserLikeItProposalsStyles.content, TableLayoutStyles.cellVerticalAlignMiddle)
            )(<.SpinnerComponent.empty),
            <.style()(WaitingForUserLikeItProposalsStyles.render[String])
          )
        }
      )
}

object WaitingForUserLikeItProposalsStyles extends StyleSheet.Inline {
  import dsl._

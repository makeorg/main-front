  ...
  lazy val reactClass: ReactClass =
    React.createClass[TriggerSignUpProps, TriggerSignUpState](
      ...
      componentDidMount = { self =>
        val onTriggerSignUp: (Location, Boolean) => Unit = {
          case (location, forceSignUp) =>
            if (!forceSignUp)
              self.setState(state => state.copy(voteCount = state.voteCount + 1))
            if (forceSignUp || self.state.voteCount != 0 && self.state.voteCount % self.props.wrapped.nVotesTriggerConnexion == 0)

...
case class ChoiceConstraint(choices: js.Array[String]) extends Constraint {
  override def validate(value: Option[String],
    ...
    if (!value.getOrElse("").isEmpty && !choices.contains(value.getOrElse(""))) {
      js.Array(ConstraintError(constraintMessages.getOrElse("invalid", "The value you selected is not a valid choice")))
    } else {
      js.Array()
    }
  }
